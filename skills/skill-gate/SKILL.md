...
**Skill Gate** provides standard-library-only static feature extraction, multi-factor
risk profiling, and harness compatibility evaluation for AI agent skills.

**Invariant:** Skill Gate is strictly static and read-only. It never executes
...
audit = skill_gate.analyze_skill("/path/to/skill", target_harness="pi")
report_dict = audit.to_dict()

# Feature extraction & deterministic hashing
features = skill_gate.extract_features("/path/to/skill")
...
compat = skill_gate.evaluate_compatibility(features, target_harness="pi")
print(compat.status)       # compatible, incompatible

# Low-level AST & lexical parsers
fm, body = skill_gate.parse_frontmatter(skill_md_content)
py_analysis = skill_gate.analyze_python_code(python_source)
sh_analysis = skill_gate.scan_shell_script(shell_source)
```
