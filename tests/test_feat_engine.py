"""Unit and integration tests for feat-engine.
...
This is body text.
"""
        fm, body = feat_engine.parse_frontmatter(content)
...
def cleanup():
    ...
    with open('output.txt', 'w') as f:
        f.write('done')
"""
        analysis = feat_engine.analyze_python_code(code)
    ...
    def test_syntax_error_resilience(self):
        ...
        analysis = feat_engine.analyze_python_code(invalid_code)
...
"""
        scan = feat_engine.scan_shell_script(script)
...
""")
    ...
    def test_vector_hash_format_and_invariance(self):
        f1 = feat_engine.extract_features(str(self.skill_dir))
        f2 = feat_engine.extract_features(str(self.skill_dir))
...
# Read only docs
""")
            features = feat_engine.extract_features(str(path))
...
""")
            features = feat_engine.extract_features(str(path))
            report = feat_engine.evaluate_compatibility(features, target_harness="pi")
...
""")
            features = feat_engine.extract_features(str(path))
            report = feat_engine.evaluate_compatibility(features, target_harness="claude")
            self.assertEqual(report.status, "incompatible")
            self.assertIn("alien_ray_gun", report.missing_tools)


class TestFeatEngineE2E(unittest.TestCase):
    def test_analyze_existing_repo_skills(self):
        # Statically analyze active repo skills
        for skill_dir in [ROOT / "skills/skill-scout", ROOT / "skills/grill-me-with-jev", ROOT / "skills/ai-native-sdlc"]:
            if skill_dir.is_dir():
                audit = feat_engine.analyze_skill(str(skill_dir), target_harness="pi")
    ...
    def test_cli_execution_json(self):
        ...
        cmd = [sys.executable, str(ROOT / "feat_engine.py"), str(scout_dir), "--json", "--check-compat", "pi"]
